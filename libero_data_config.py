# Custom data config for libero-hfvla dataset
# Based on examples/Libero/custom_data_config.py but adapted for the dual camera setup
...
class LiberoDataConfig(BaseDataConfig):
    """
    Data config for libero-hfvla dataset with two cameras.

    Dataset structure:
    - State: observation.state (8 dims) -> x, y, z, roll, pitch, yaw, gripper (2 dims)
    - Action: action (7 dims) -> x, y, z, roll, pitch, yaw, gripper (1 dim)
    - Video: video.image (agentview), video.image2 (second camera)
